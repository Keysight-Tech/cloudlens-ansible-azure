...
if [[ "$CHAIN_SENSORS" == "true" ]] || [[ "$CHAIN_SENSORS" == "write_yaml_only" ]]; then
  ...
  if [[ "$DRY_RUN" == "true" ]]; then
    ...
    # Gap fix 1: include tenant_id so Azure SDK auth never silently falls back
    TENANT_ID="$(az account show --query tenantId -o tsv 2>/dev/null || echo '')"

    # Gap fix 2: detect Linux SSH auth. If ~/.ssh/id_rsa exists, use it.
    # Otherwise default to ansible_password = the OS-level SSH password the
    # customer typed earlier. Either way the yaml is ready to run as-is.
    if [[ -f "$HOME/.ssh/id_rsa" ]]; then
      LINUX_AUTH_LINE="ssh_key_file: \"~/.ssh/id_rsa\""
      AUTH_NOTE="# Linux: SSH key auth (~/.ssh/id_rsa found on this machine)."
    else
      LINUX_AUTH_LINE="ansible_password: \"${ADMIN_PASSWORD}\""
      AUTH_NOTE="# Linux: password auth (no SSH key found). For SECURITY, switch to"$'\n'"# ssh_key_file once your workload VMs are configured with a public key."
    fi
...
# Uncomment for Windows VMs (set the password your Win VMs actually use):
# windows:
#   ansible_user: "azureuser"
#   ansible_password: "your-windows-vm-password"
YAML
    ok "Wrote customer_input.yaml (with tenant_id, Linux auth, Windows stub)"

    # Gap fix 3: auto-locate or clone the repo so quickstart.sh actually runs.

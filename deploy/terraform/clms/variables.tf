variable "subscription_id" {
  description = "Azure subscription ID where CLMS will be deployed."
  type        = string
}
...
variable "vm_name" {
  description = "Name of the CLMS VM. Also used as a prefix for NIC, NSG, public IP, and (when creating) VNet."
  type        = string
  default     = "clms"
}

variable "admin_username" {
  description = "Admin username for OS-level access on the CLMS VM."
  type        = string
  default     = "azureuser"
}

variable "admin_password" {
  description = "Admin password for the CLMS VM. Must be 12+ chars with mixed case, digit, and symbol."
  type        = string
  sensitive   = true
}

variable "vm_size" {
  description = "VM size for the CLMS VM. D4s_v5 (4 vCPU, 16 GB) suits most deployments."
  type        = string
...
variable "existing_subnet_name" {
  description = "Name of an existing subnet to attach the CLMS NIC to. Leave blank to create a new subnet."
  type        = string
  default     = ""
}

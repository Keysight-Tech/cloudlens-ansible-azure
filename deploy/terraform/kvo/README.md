...
Terraform module that deploys a single Keysight Vision Orchestrator (KVO) VM from the Azure Marketplace, mirroring the `deploy/kvo-marketplace.json` ARM template. Same marketplace image, same outputs, but IaC-friendly for pipelines, version control, and repeatable deployments.
...
- Marketplace terms accepted for the KVO image (see command at the bottom)
...
After ~15 minutes the KVO UI is reachable at the `kvo_ui_url` output.
...
| `vm_name` | `kvo` | VM name and resource prefix |
...
| Output | Meaning |
|---|---|
| `kvo_public_ip` | Public IP of the KVO VM |
| `kvo_ui_url` | HTTPS URL for the KVO web UI |
| `default_credentials` | `admin / Cl0udLens@dm!n` (change on first login) |
| `next_step` | What to do once KVO is up (create project, copy key) |
